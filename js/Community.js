...
(function () {
  ...
  function renderMonthFilter() {
    ...
    /* 모바일 combobox 동기화 — 입력 필드 라벨 + 건수 칩 */
    var ddInput = $('monthDdInput'), ddCount = $('monthDdCount');
    if (ddInput && document.activeElement !== ddInput) ddInput.value = monthComboLabel(patchMonth);
    if (ddCount) ddCount.textContent = (patchMonth === 'all' ? S.patches.length : (months[patchMonth] || 0)) + '건';
  }
  function monthComboLabel(k) { return k === 'all' ? '전체 월' : k.replace('-', '년 ') + '월'; }
  /* 월 검색 매칭 — "2024", "3", "03", "3월", "2024년", "202403" 등 유연하게 */
  function monthMatches(k, q) {
    if (!q) return true;
    var s = q.replace(/\s+/g, '').toLowerCase();
    if (k === 'all') return ('전체월').indexOf(s) > -1 || s === 'all';
    var parts = k.split('-');
    var y = parts[0] || '';
    var m = String(parseInt(parts[1], 10) || '');
    var label = (y + '년' + m + '월').toLowerCase();
    if (label.indexOf(s) > -1) return true;
    if (/^\d+$/.test(s)) return y.indexOf(s) === 0 || m === s || ('0' + m) === s;
    return false;
  }
  /* 모바일 월별 combobox — 입력 필터링 + 키보드 내비게이션 */
  function bindMonthDd() {
    var wrap = $('monthDd');
    if (!wrap) return;
    var input = $('monthDdInput'), list = $('monthDdList'), chev = $('monthDdChev'), combo = $('monthCombo');
    var st = { open: false, active: -1, opts: [] };

    function currentData() {
      var months = {};
      S.patches.forEach(function (p) { var m = String(p.date || '').slice(0, 7); if (m) months[m] = (months[m] || 0) + 1; });
      var keys = Object.keys(months).sort().reverse();
      return { months: months, keys: keys };
    }
    function render(query) {
      ...
      function push(k, count) {
        var id = 'mopt-' + k.replace(/[^a-z0-9]/gi, '');
        st.opts.push({ key: k, id: id });
        html += '<div class="month-dd-opt' + (patchMonth === k ? ' is-on' : '') + '" id="' + id + '" data-m="' + k + '" role="option" aria-selected="' + (patchMonth === k) + '">' +
          '<span>' + monthComboLabel(k) + '</span><small>' + count + '</small></div>';
      }
      if (monthMatches('all', query)) push('all', S.patches.length);
      d.keys.forEach(function (k) { if (monthMatches(k, query)) push(k, d.months[k]); });
      list.innerHTML = st.opts.length ? html : '<div class="month-dd-empty">조건에 맞는 월이 없습니다</div>';
      st.active = st.opts.length ? 0 : -1;
      paintActive();
    }
    function paintActive() {
      list.querySelectorAll('.month-dd-opt').forEach(function (o, i) { o.classList.toggle('is-active', i === st.active); });
      var act = st.opts[st.active];
      input.setAttribute('aria-activedescendant', act ? act.id : '');
      var ao = list.querySelector('.month-dd-opt.is-active');
      if (ao && ao.scrollIntoView) ao.scrollIntoView({ block: 'nearest' });
    }
    function openList() {
      if (st.open) return;
      st.open = true;
      ...
      render(input.value === monthComboLabel(patchMonth) ? '' : input.value.trim());
    }
    function closeList(restore) {
      ...
      if (restore) input.value = monthComboLabel(patchMonth);
    }
    function select(key) {
      patchMonth = key;
      patchPage = 1;
      input.value = monthComboLabel(key);
      var d = currentData();
      var cnt = key === 'all' ? S.patches.length : (d.months[key] || 0);
      var chip = $('monthDdCount');
      if (chip) chip.textContent = cnt + '건';
      closeList(false);
      renderMonthFilter();
      renderPatchList();
    }
    input.addEventListener('focus', function () { openList(); });
    input.addEventListener('click', function (e) { e.stopPropagation(); if (!st.open) openList(); });
    input.addEventListener('input', function () { openList(); render(input.value.trim()); });
    input.addEventListener('keydown', function (e) {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        if (!st.open) openList();
        else if (st.opts.length) { st.active = (st.active + 1) % st.opts.length; paintActive(); }
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        if (st.open && st.opts.length) { st.active = st.active <= 0 ? st.opts.length - 1 : st.active - 1; paintActive(); }
      } else if (e.key === 'Enter') {
        e.preventDefault();
        var o = st.opts[st.active];
        if (o) select(o.key);
      } else if (e.key === 'Escape') {
        e.preventDefault();
        closeList(true);
        input.blur();
      } else if (e.key === 'Home' && st.open && st.opts.length) {
        e.preventDefault(); st.active = 0; paintActive();
      } else if (e.key === 'End' && st.open && st.opts.length) {
        e.preventDefault(); st.active = st.opts.length - 1; paintActive();
      }
    });
    list.addEventListener('mousedown', function (e) { e.preventDefault(); });
    list.addEventListener('click', function (e) {
      var o = e.target.closest('.month-dd-opt');
      if (!o) return;
      select(o.getAttribute('data-m'));
    });
    chev.addEventListener('mousedown', function (e) { e.preventDefault(); });
    chev.addEventListener('click', function (e) {
      e.stopPropagation();
      if (st.open) { closeList(true); input.blur(); } else { input.focus(); openList(); }
    });
    document.addEventListener('click', function (e) {
      if (st.open && (!e.target.closest || !e.target.closest('#monthDd'))) closeList(true);
    });
  }
